...
function Options2NavTimer:Constructor(navWin, winIdx, timerIdx, timerData, key, expanded, depth)
    ...
    self.toggle:SetBackColor(timerData.enabled == true and COL_ON or COL_OFF)
    self.toggle.MouseClick = function()
        timerData.enabled = not timerData.enabled
        self.toggle:SetBackColor(timerData.enabled and COL_ON or COL_OFF)
        Options.SaveData()
...
function Options2NavTimer:SizeChanged()
    ...
    self.label:SetWidth(w - cx - ARROW_W - reserve - TOG_W - 2)
    if self._has_icon then
        self.icon_preview:SetLeft(w - ICON_SIZE - ICON_MARGIN - TOG_W)
    end
