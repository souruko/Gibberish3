...
local ARROW_W = 18
local BADGE_W = 160
local COLOR_CT = Turbine.UI.Color(0.70, 0.42, 0.85)
local TOG_W   = 18
local TOG_S   = 10
...
Options2NavCondTrigger = class(Turbine.UI.Control)
function Options2NavCondTrigger:Constructor(navWin, trigData, trigType, trigIdx, winIdx, timerIdx, condIdx, key, depth)
    ...
    self.stripe:SetBackColor(COLOR_CT)
    self.stripe:SetMouseVisible(false)

    local win_line_x = STRIPE + (self.depth - 3) * INDENT + math.floor((ARROW_W - 2) / 2)
...
function Options2NavCondTrigger:SizeChanged()
...
function Options2NavCondTrigger:SetSelected(v)
    self.selected = v
    self:SetBackColor(v and Options.Defaults.window.w_window_select or nil)
end

function Options2NavCondTrigger:GetKey()       return self.key end
function Options2NavCondTrigger:IsExpandable() return false end
function Options2NavCondTrigger:SetExpanded(v) end

function Options2NavCondTrigger:Refresh(_, depth)
    if depth ~= self.depth then
        self.depth = depth
        local cx = STRIPE + depth * INDENT
        local win_line_x = STRIPE + (depth - 3) * INDENT + math.floor((ARROW_W - 2) / 2)
        self.win_line:SetPosition(win_line_x, 0)
        self.spacer:SetPosition(cx, 0)
        self.label:SetPosition(cx + ARROW_W, 0)
    end
